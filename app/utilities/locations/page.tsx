...
const CO_COLOR: Record<Company, string> = {
  afs: 'bg-blue-100 text-blue-700 border-blue-200',
  tnt: 'bg-amber-100 text-amber-700 border-amber-200',
  zfs: 'bg-emerald-100 text-emerald-700 border-emerald-200',
}
      ...
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-md max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-6 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              <label className="block text-xs font-semibold text-gray-500 mb-1">Region / Province</label>
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto">
      ...
      {loading ? (
        ...
        <div className="flex items-center justify-center h-40 text-sm text-gray-400">No locations found.</div>
      ) : (
            ...
            <div key={co}>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {items.map(loc => {
                  ...
                  return (
                    <div key={loc.id} className="bg-white rounded-2xl border border-gray-100 p-5 hover:shadow-md transition-shadow group">
                      ...
                      {loc.address && (
                        <p className="text-xs text-gray-400 mb-3 leading-relaxed">{loc.address}</p>
                      )}

                      <div className="flex gap-4 mb-3">
                        <div className="text-center">
                          <p className="text-lg font-bold text-gray-900">{s.vendorCount}</p>
                          <p className="text-[10px] text-gray-400 uppercase tracking-wide">Vendors</p>
                        </div>
                        <div className="text-center">
                          <p className="text-lg font-bold text-gray-900">{s.billCount}</p>
                          <p className="text-[10px] text-gray-400 uppercase tracking-wide">Bills</p>
                        </div>
                      </div>

                      {loc.notes && (
                        <p className="text-xs text-gray-400 italic border-t border-gray-100 pt-2">{loc.notes}</p>
                      )}
                      ...
                      <div className="flex items-center gap-2 mt-3 pt-3 border-t border-gray-100 opacity-0 group-hover:opacity-100 transition-opacity">
                        <button onClick={() => { setEditLoc(loc); setShowModal(true) }}
                          className="text-xs text-gray-500 hover:text-gray-800 transition-colors font-medium">Edit</button>
                        {delConfirm === loc.id ? (
                          <div className="flex gap-1 ml-auto">
                            <button onClick={() => deleteLocation(loc.id)} className="text-xs text-red-600 font-medium hover:text-red-800">Delete</button>
                            <button onClick={() => setDelConfirm(null)} className="text-xs text-gray-400">Cancel</button>
                          </div>
                        ) : (
                          <button onClick={() => setDelConfirm(loc.id)} className="text-xs text-red-400 hover:text-red-600 transition-colors ml-auto">Delete</button>
                        )}
                      </div>
                    </div>
                  )
