...
export async function GET(req: NextRequest) {
  ...
  const monthlyDate = new Date(`${month}T00:00:00Z`)
  const monthlyBills = await auth.db.from('utility_bills').select('id,provider,utility_name').eq('billing_year', monthlyDate.getUTCFullYear()).eq('billing_month', monthlyDate.getUTCMonth() + 1).order('provider')
