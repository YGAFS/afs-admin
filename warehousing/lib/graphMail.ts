...
async function _postMail(accessToken: string, payload: MailPayload) {
  ...
  // Convert plain text body to HTML. Deliberately no injected signature block here —
  // unlike the HR app's single fixed sender, "Send to Bookkeeping" can be clicked by
  // any purchasing/admin user, so a hardcoded name would misattribute the email.
  // Outlook appends the sender's own account signature automatically server-side
  // when one is configured, same as a normal compose.
  const bodyHtml = payload.body
    .replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;')
    .replace(/\n/g, '<br>')
  const fullHtml = `<div style="font-family: Aptos, Arial, sans-serif; font-size: 13px;">${bodyHtml}</div>`
...
/** Called on the request-detail page mount — sends any pending email left over from redirect auth */
export async function sendPendingMailAfterRedirect(): Promise<{ sent: boolean; error?: string }> {
  ...
  try {
    ...
    const accounts = msal.getAllAccounts()
    if (!accounts.length) return { sent: false, error: cbError ? `[콜백 오류] ${cbError}` : '인증 후 계정을 찾을 수 없습니다' }
    const tokenRes = await msal.acquireTokenSilent({ scopes: MAIL_SCOPES, account: accounts[0] })
    await _postMail(tokenRes.accessToken, payload)
    return { sent: true }
  } catch (e) {
